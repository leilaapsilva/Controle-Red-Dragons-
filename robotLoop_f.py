...
if (distTR < 0.2) and (distTR>0.15): # redução da velocidade do robo p/atingir o alvo
    ...
    if distTR < 0.15: # mudança de alvo robô vai para o gol
        ...
        while distGR < 2  and flag and distTR <0.15: # robo vai para o gol
                    ...
                    distGR = abs(sqrt((xg-x)^2 + (yg-y)^2 )) # distance between target and robot
                        ...
                        ylabel('Y(m)')
                        drawnow
                        if distGR < 0.08 and distGR > 0.05: # redução da velocidade perto do gol
                            Kvr  = 1
                            vr = Kvr*distGR*vrMax
                        else:
